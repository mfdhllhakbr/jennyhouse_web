...
if (isset($_POST["verification"])) {
    $update = "UPDATE `transaction` SET `status` = 'Terverifikasi' WHERE `transaction`.`transactionid` = $transid;";
    mysqli_query($conn, $update);
    if (mysqli_affected_rows($conn) > 0) {
        echo "<script>alert(`Transaksi Terverifikasi`);</script>";
    }
...
<body>
    ...
    <div class="container w-50 mt-3 mb-3">
        ...
        <form action="" method="POST">
            <button type="submit" class="btn btn-primary" name="verification">Verification</button>
        </form>
